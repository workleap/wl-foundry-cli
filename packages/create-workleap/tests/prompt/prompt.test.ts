...
describe("Given Prompt.MultiSelect", () => {
    ...
    it.each([
        ...
        async (message, initialValue) => {
            ...
            const result = await prompt.MultiSelect(message, options, [initialValue?.value]);
    ...
    test("When multiselect is required Then work", async () => {
        ...
        const result = await prompt.MultiSelect(message, options, undefined, undefined, true);
...
describe("Given Prompt.Outro", () => {
    ...
    test("When message Then work", () => {
        ...
        expect(prompts.outro).toHaveBeenCalledWith(expect.stringContaining(outroText));
    });
    ...
    test("When message with color Then work", () => {
        ...
        expect(prompts.outro).toHaveBeenCalledTimes(1);
        expect(prompts.outro).toHaveBeenCalledWith(expect.stringContaining(outroText));
        expect(prompts.outro).toHaveBeenCalledWith(expect.stringContaining(BlackColorCode));
    });
});
